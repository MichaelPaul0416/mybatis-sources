...
public class XMLConfigBuilder extends BaseBuilder {
  ...
  private void parseConfiguration(XNode root) {
    try {
      ...
      loadCustomVfs(settings);//加载vfsImpl属性指定得类实现
      typeAliasesElement(root.evalNode("typeAliases"));//key=name,value=class类对象
      pluginElement(root.evalNode("plugins"));//加载插件
      //ObjectFactory，只支持构造器初始化，可以是有参或者无参
  ...
  private Properties settingsAsProperties(XNode context) {
    ...
    Properties props = context.getChildrenAsProperties();//将子标签以key-value的形式读取进来【<setting name="cacheEnabled" value="true"/>】
    // Check that all settings are known to the configuration class
    MetaClass metaConfig = MetaClass.forClass(Configuration.class, localReflectorFactory);
    for (Object key : props.keySet()) {
      if (!metaConfig.hasSetter(String.valueOf(key))) {//Configuration类没有setter方法
  ...
  private void typeAliasesElement(XNode parent) {
    if (parent != null) {
      for (XNode child : parent.getChildren()) {
        if ("package".equals(child.getName())) {//<package name="org.apache.ibatis.submitted.permissions" />
  ...
  private void pluginElement(XNode parent) throws Exception {
    if (parent != null) {
      ...
      for (XNode child : parent.getChildren()) {
        ...
        //find class in classloaders and register it
  ...
  private void propertiesElement(XNode context) throws Exception {
    if (context != null) {
      ...
      if (resource != null) {
        defaults.putAll(Resources.getResourceAsProperties(resource));//引入外部properties文件
      } else if (url != null) {
        defaults.putAll(Resources.getUrlAsProperties(url));
      }
      Properties vars = configuration.getVariables();//之前通过SqlSessionFactoryBuilder.build(x,x,props)传递进来的properties属性
      if (vars != null) {
        defaults.putAll(vars);
      }
      parser.setVariables(defaults);
      configuration.setVariables(defaults);//在原先的基础上再并入resource/url指定的资源文件的属性
    }
  }

  private void settingsElement(Properties props) throws Exception {
    /**
     * 下列数据具体参考配置文件【D:\Program Files\IdeaProjects\mybatis-3-master\src\site\zh\xdoc\configuration.xml】
     */
    ...
    if (context != null) {
      ...
      for (XNode child : context.getChildren()) {
        ...
        if (isSpecifiedEnvironment(id)) {
          ...
          configuration.setEnvironment(environmentBuilder.build());//简单理解为environment=transactionFactory+datasource
        }
      }
    }
  }
